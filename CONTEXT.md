...
- **Never link to, index, or sitemap `/parents` or `/staff`.** Keep `<meta name="robots" content="noindex,nofollow">` on both login pages and keep them out of public links and the sitemap. Allow crawling of the login pages so search engines can read `noindex`; do not block them in `robots.txt`. Private API data and photos must remain protected by authentication and authorization.
